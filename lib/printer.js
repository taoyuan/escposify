...
Printer.prototype.lineSpacing = function (n) {
...
Printer.prototype.bitimage = function (image, density) {
  ...
  density = density || 'd24';
  if (density === 's8') {
    result = image.toBitmap(8);
    header = commands.BITMAP_FORMAT.BITMAP_8_SINGLE_DENSITY;
    n = 1;
  } else if (density === 'd8') {
    result = image.toBitmap(8);
    header = commands.BITMAP_FORMAT.BITMAP_8_DOUBLE_DENSITY;
    n = 1;
  } else if (density === 's24') {
    result = image.toBitmap(24);
    header = commands.BITMAP_FORMAT.BITMAP_24_SINGLE_DENSITY;
    n = 3;
  } else {
    result = image.toBitmap(24);
    header = commands.BITMAP_FORMAT.BITMAP_24_DOUBLE_DENSITY;
    n = 3;
  }
...
// set line spacing to 0
  ...
  _.forEach(result.data, function (line) {
    that.buffer.write(header, 'binary');
